"""CLI-тесты точки входа `atomno-mcp-erid`."""

from __future__ import annotations

import pytest

from mcp_erid import __version__
from mcp_erid.server import (
    _DEFAULT_HTTP_HOST,
    _DEFAULT_HTTP_PORT,
...
class TestInvalidEnvBailsOutCleanly:
    def test_invalid_env_and_no_cli_flag_exits_two(self, monkeypatch, capsys) -> None:
        monkeypatch.setenv("MCP_ERID_LOG_LEVEL", "VERBOSE")
        with pytest.raises(SystemExit) as exc_info:
            main([])
        assert exc_info.value.code == 2
        err = capsys.readouterr().err
        assert "MCP_ERID_LOG_LEVEL" in err
